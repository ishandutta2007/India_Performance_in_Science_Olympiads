...
def plot_olympiad(name):
    if name not in olympiad_data:
        print(f"Error: {name} is not a valid Olympiad. Choose from {list(olympiad_data.keys())}")
        sys.exit(1)
    ...
    if len(peaks) == 1:
        ...
        xytext_offset = 5
        if name == "IMO": xytext_offset = 5
        elif name == "IBO": xytext_offset = 5
        elif name == "IChO": xytext_offset = 5
        elif name == "IOI": xytext_offset = 5
        
        plt.annotate(
            f"🏆 Historic Peak!\nRank {int(best_data['Rank'])} of {int(best_data['Total_Countries'])}\n({fmt_pct}th Percentile)",
            xy=(best_year, best_data["Percentile"]),
            xytext=(best_year - xytext_offset, best_data["Percentile"] - (6 if name != "IOI" else 2)),
            arrowprops=dict(
